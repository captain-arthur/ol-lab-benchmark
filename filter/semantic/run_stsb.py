...
- sentence-transformers/stsb로 3단계 실험을 순차 수행
"""
...
def load_stsb_dataset():
    ...
    try:
        ...
        print("✅ MTEB/STS-B 데이터셋 로딩 성공")
    except Exception as e:
        print(f"⚠️  MTEB/STS-B 로딩 실패: {e}")
        try:
            # 직접 로딩 시도
            dataset = load_dataset("sentence-transformers/stsbenchmark")
            print("✅ sentence-transformers/STS-B 데이터셋 로딩 성공")
        except Exception as e2:
            print(f"❌ STS-B 데이터셋 로딩 실패: {e2}")
            raise
    ...
    for item in train_data:
        text = item["sentence1"]
        label = item["genre"]  # genre를 라벨로 사용
...
def run_stsb():
    ...
    corpus_texts, corpus_labels, query_texts, query_labels = load_stsb_dataset()
    ...
    # 결과를 파일로 저장
    baseline_path = os.path.join(OUT_DIR, "stsb_sbert_cosine.json")
    ...
    # 결과를 파일로 저장
    anchor_path = os.path.join(OUT_DIR, "stsb_sbert_anchors.json")
    ...
    # 결과를 파일로 저장
    ce_path = os.path.join(OUT_DIR, "stsb_sbert_anchors_ce.json")
