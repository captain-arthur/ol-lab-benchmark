...
"""
run_ms_marco.py - MS MARCO Passage Ranking Benchmark (Semantic Filter)
...
def iter_ms_marco_query_groups(split: str = "validation",
                               max_queries: int = 20) -> Iterable[Tuple[str, List[str], List[int], str]]:
    """MS MARCO 데이터 스트리밍 로더 (qrels 기반 정상 라벨링)"""
    try:
    ...
    for item in ds:
        ...
        query_id = item.get("query_id", f"q_{count}")  # 쿼리 ID 추가
        ...
        yield query, keep_texts, keep_labels, query_id
        count += 1
    ...
    for i, (q, qid) in enumerate(zip(query_texts, qids)):
        ...
        if dropped_docs:
            ...
            for k, doc_id in enumerate(dropped_docs):
                doc_idx = candidate_pool.index(doc_id)
                ce_scores[doc_idx] = dropped_ce_scores[k]
                all_ce_scores.append(dropped_ce_scores[k])
    ...
    for i, (q, qid) in enumerate(zip(query_texts, qids)):
        ...
        if len(anchors) > 0:
            ...
            for j, doc_id in enumerate(candidate_pool):
                sbert_score = sbert_scores[j]
                ce_score = ce_scores[j]
                if sbert_score < 0.3 and ce_score < 0.3:
                    final_dropped_sim.append(doc_id)
        ...
        # CBC 기반 임계값 (바닥값 부여) - 최적화된 바닥값으로 조정
...
def run_semantic_benchmark():
    """MS MARCO 유사도 필터 벤치마크 실행"""
    print("=" * 60)
    print("MS MARCO Semantic Filter Benchmark")
    print("=" * 60)
    print("Configuration:")
    ...
    for query, passages, labels, query_id in iter_ms_marco_query_groups(
    ...
    # 캐시 디렉토리 설정 (.cache/semantic/ms_marco/)
    cache_dir = ".cache/semantic/ms_marco"
...
def print_comparison_summary(results: Dict[str, Dict[str, Any]]):
    ...
    print("-" * 80)
    print(f"{'지표':<20} {'① SBERT':<15} {'② SBERT+CE':<15} {'③ SBERT+CE+LLM':<18} {'④ SBERT+CE+LLM+CBC':<20}")
    print("-" * 80)
